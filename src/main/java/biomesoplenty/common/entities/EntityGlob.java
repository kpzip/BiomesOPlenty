...
public class EntityGlob extends EntityLiving implements IMob {
    ...
    public boolean getCanSpawnHere() {
        ...
        if (worldObj.getWorldInfo()
            .getTerrainType()
            .handleSlimeSpawnReduction(rand, worldObj)) return false;
        else {
            if (this.getGlobSize() == 1 || worldObj.difficultySetting != EnumDifficulty.PEACEFUL) {
                BiomeGenBase biomegenbase = worldObj
                    .getBiomeGenForCoords(MathHelper.floor_double(posX), MathHelper.floor_double(posZ));

                if (biomegenbase == BiomeGenBase.swampland && posY > 50.0D
                    && posY < 70.0D
                    && rand.nextFloat() < 0.5F
                    && rand.nextFloat() < this.worldObj.getCurrentMoonPhaseFactor()
                ...
                if (rand.nextInt(10) == 0 && chunk.getRandomWithSeed(987234911L)
                    .nextInt(10) == 0 && posY < 40.0D) return super.getCanSpawnHere();
            }
