...
public class EntityAITemptArmor extends EntityAIBase {
    ...
    public EntityAITemptArmor(EntityCreature entityCreature, float speedTowardsTarget, Item armorItem, int armorMeta,
        boolean scaredByPlayerMovement) {
    ...
    public boolean shouldExecute() {
        ...
        if (temptingPlayer == null) return false;
            ...
            return itemstack == null ? false
                : (itemstack.getItem() == armorItem) && (itemstack.getItemDamage() >= armorMeta);
        }
    }
        ...
        if (scaredByPlayerMovement) {
            if (temptedEntity.getDistanceSqToEntity(temptingPlayer) < 36.0D) {
                ...
                if (Math.abs(temptingPlayer.rotationPitch - field_75278_f) > 5.0D
                    || Math.abs(temptingPlayer.rotationYaw - field_75279_g) > 5.0D) return false;
            } else {
    ...
    public void startExecuting() {
        ...
        targetZ = temptingPlayer.posZ;
        isRunning = true;
        field_75286_m = temptedEntity.getNavigator()
            .getAvoidsWater();
        temptedEntity.getNavigator()
    ...
    public void updateTask() {
        temptedEntity.getLookHelper()
            .setLookPositionWithEntity(temptingPlayer, 30.0F, temptedEntity.getVerticalFaceSpeed());
        ...
        if (temptedEntity.getDistanceSqToEntity(temptingPlayer) < 6.25D) {
            ...
            temptedEntity.getNavigator()
                .tryMoveToEntityLiving(temptingPlayer, speedTowardsTarget);
        }
    }
