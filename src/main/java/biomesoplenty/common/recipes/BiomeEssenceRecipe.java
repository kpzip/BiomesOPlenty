...
public class BiomeEssenceRecipe implements IRecipe {
    ...
    public boolean matches(InventoryCrafting inventoryCrafting, World world) {
        ...
        if (biomeRadar != null && biomeEssence != null) {
            if (!biomeEssence.hasTagCompound() || !biomeEssence.getTagCompound()
                .hasKey("biomeID")) return false;

            int biomeID = biomeEssence.getTagCompound()
                .getInteger("biomeID");

            if (!biomeRadar.hasTagCompound()) biomeRadar.setTagCompound(new NBTTagCompound());

            biomeRadar.getTagCompound()
                .setInteger("biomeIDToFind", biomeID);
            biomeRadar.getTagCompound()
                .setBoolean("foundBiome", false);
