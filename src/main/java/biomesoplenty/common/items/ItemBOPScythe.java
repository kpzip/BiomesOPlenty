...
public class ItemBOPScythe extends Item {
    ...
    public boolean onBlockDestroyed(ItemStack itemstack, World world, Block block, int x, int y, int z,
        ...
        if (toolMaterial == ToolMaterial.IRON || toolMaterial == ToolMaterial.GOLD) {
            if (block != null) {
                if (block.isLeaves(world, x, y, z)) height = 4;
            }

            radius = 4;
        } else if (toolMaterial == ToolMaterial.EMERALD) {
            if (block != null) {
                if (block.isLeaves(world, x, y, z)) height = 5;
            }

            radius = 5;
        } else if (toolMaterial == BOPItemHelper.toolMaterialAmethyst) {
            if (block != null) {
                if (block.isLeaves(world, x, y, z)) height = 6;
            }

            radius = 6;
        } else {
            if (block != null) {
                if (block.isLeaves(world, x, y, z)) height = 0;
            }
        }
