...
public class BlockFlesh extends Block {
    ...
    public void randomDisplayTick(World world, int x, int y, int z, Random random) {
        ...
        if (random.nextInt(4) == 0) {
            // TODO: getIdFromBlock()
            world.spawnParticle(
                "blockcrack_" + String.valueOf(Block.getIdFromBlock(this)) + "_0",
                x + random.nextFloat(),
                y - 0.4F,
                z + random.nextFloat(),
                0.0D,
    ...
    public void onEntityCollidedWithBlock(World world, int x, int y, int z, Entity entity) {
        if (entity instanceof EntityPlayer) {
            ...
            /*
             * TODO FEATURE if (inventory.armorInventory[0] != null && inventory.armorInventory[0].itemID ==
             * Items.wadingBoots.get().itemID)
             * {
             * return;
             * }
             */
