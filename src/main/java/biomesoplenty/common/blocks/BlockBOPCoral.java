...
public class BlockBOPCoral extends BOPBlock {
    ...
    private static final String[] coral = new String[] { "kelpbottom", "kelpmiddle", "kelptop", "kelpsingle",
        "pinkcoral", "orangecoral", "bluecoral", "glowcoral", "algae" };
    ...
    public boolean canReplace(World world, int x, int y, int z, int side, ItemStack itemStack) {
        return world.getBlock(x, y + 1, z) == Blocks.water
            && this.canBlockStay(world, x, y, z, itemStack.getItemDamage());
    }
    ...
    public void onNeighborBlockChange(World world, int x, int y, int z, Block neighborBlock) {
        ...
        if (category == CoralCategory.CAT1 && (meta >= 8 && meta <= 11)) {
            ...
            } else if (world.getBlock(x, y + 1, z) != this && world.getBlock(x, y - 1, z) == this) // Convert to top
                                                                                                   // piece
            {
    ...
    public boolean canBlockStay(World world, int x, int y, int z, int metadata) {
        ...
        if (category == CoralCategory.CAT1) {
            switch (metadata) {
                ...
                case 11:
                    return block == this || block == Blocks.dirt
                        || block == Blocks.sand
                        || block == Blocks.sponge
                        || block == Blocks.stone
                        || block == Blocks.clay
