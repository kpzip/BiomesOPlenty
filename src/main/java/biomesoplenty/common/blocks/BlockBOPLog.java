...
public class BlockBOPLog extends Block {
    ...
    private static final String[] types = new String[] { "sacredoak", "cherry", "dark", "fir", "holy", "magic",
    ...
    public IIcon getIcon(int side, int meta) {
        ...
        if (pos == 0 && (side == 1 || side == 0) || pos == 4 && (side == 5 || side == 4)
            || pos == 8 && (side == 2 || side == 3)) return logHearts[(getTypeFromMeta(meta) + category.ordinal() * 4)];
        else return textures[(getTypeFromMeta(meta) + category.ordinal() * 4)];
    }
