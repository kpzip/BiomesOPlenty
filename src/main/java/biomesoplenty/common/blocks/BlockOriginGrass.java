...
public class BlockOriginGrass extends Block {
    ...
    public IIcon getIcon(int side, int meta) {
        if (side < 0 || side >= this.icon.length) side = 1;
        ...
        if (player.getCurrentEquippedItem() != null) {
            if (player.getCurrentEquippedItem()
                ...
                world.playSoundEffect(
                    ...
                    y + 0.5F,
                    z + 0.5F,
                    tilledField.stepSound.func_150496_b(),
                    (tilledField.stepSound.getVolume() + 1.0F) / 2.0F,
                    tilledField.stepSound.getPitch() * 0.8F);
        ...
        if (!world.isRemote) {
            if (world.getBlockLightValue(x, y + 1, z) < 4 && world.getBlockLightOpacity(x, y + 1, z) > 2) {
                ...
                for (int var6 = 0; var6 < 4; ++var6) {
                    ...
                    // TODO: getBlock()
                    if (world.getBlock(rX, rY, rZ) == Blocks.dirt && world.getBlockLightValue(rX, rY + 1, rZ) >= 4
                        && world.getBlockLightOpacity(rX, rY + 1, rZ) <= 2) {
                        // TODO: setBlock()
