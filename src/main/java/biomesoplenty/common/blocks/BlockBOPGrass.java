...
public class BlockBOPGrass extends Block {
    ...
    public boolean isFireSource(World world, int x, int y, int z, ForgeDirection side) {
        ...
        if (metadata == 0) {
            if (side == ForgeDirection.UP && world.provider.dimensionId == -1) return true;
    ...
    public void randomDisplayTick(World world, int x, int y, int z, Random random) {
        ...
        if (world.getBlockMetadata(x, y, z) == 1) {
            if (random.nextInt(4) == 0) {
                world
                    .spawnParticle("smoke", x + random.nextFloat(), y + 1.1F, z + random.nextFloat(), 0.0D, 0.0D, 0.0D);
            }

            if (random.nextInt(6) == 0) {
                world
                    .spawnParticle("flame", x + random.nextFloat(), y + 1.1F, z + random.nextFloat(), 0.0D, 0.0D, 0.0D);
            }
        }
    }
            ...
            if (!world.isRemote) {
                if (world.getBlockLightValue(x, y + 1, z) < 4 && world.getBlockLightOpacity(x, y + 1, z) > 2) {
                    ...
                    for (int var6 = 0; var6 < 4; ++var6) {
                        ...
                        // TODO: getBlock()
                        if (world.getBlock(rX, rY, rZ) == Blocks.end_stone
                            && world.getBlockLightValue(rX, rY + 1, rZ) >= 4
