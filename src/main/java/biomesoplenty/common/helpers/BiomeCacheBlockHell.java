...
public class BiomeCacheBlockHell {
    ...
    public BiomeCacheBlockHell(BiomeCacheHell par1BiomeCache, int par2, int par3) {
        ...
        xPosition = par2;
        zPosition = par3;
        BiomeCacheHell.getChunkManager(par1BiomeCache)
            .getTemperatures(temperatureValues, par2 << 4, par3 << 4, 16, 16);
        BiomeCacheHell.getChunkManager(par1BiomeCache)
            .getRainfall(rainfallValues, par2 << 4, par3 << 4, 16, 16);
        BiomeCacheHell.getChunkManager(par1BiomeCache)
            .getBiomeGenAt(biomes, par2 << 4, par3 << 4, 16, 16, false);
    }
