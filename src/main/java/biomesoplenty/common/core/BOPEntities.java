...
public class BOPEntities {
    ...
    private static void registerEntities() {
        EntityRegistry.registerModEntity(
            EntityDart.class,
            "dart",
            BOPConfigurationIDs.entityDartID,
            BiomesOPlenty.instance,
            80,
            3,
            true);
        EntityRegistry.registerModEntity(
            EntityMudball.class,
            "mudball",
            BOPConfigurationIDs.entityMudballID,
            BiomesOPlenty.instance,
            80,
            3,
            true);
        ...
        if (BOPConfigurationIDs.jungleSpiderID > 0) {
            EntityRegistry.registerModEntity(
                EntityJungleSpider.class,
                "JungleSpider",
                BOPConfigurationIDs.jungleSpiderID,
                BiomesOPlenty.instance,
                80,
                3,
                true);
        ...
        if (BOPConfigurationIDs.rosesterID > 0) {
            EntityRegistry.registerModEntity(
                EntityRosester.class,
                "Rosester",
                BOPConfigurationIDs.rosesterID,
                BiomesOPlenty.instance,
                80,
                3,
                true);
        ...
        if (BOPConfigurationIDs.birdID > 0) {
            ...
            /*
             * TODO: FEATURE if (Biomes.promisedLandForest.isPresent() && Biomes.promisedLandSwamp.isPresent() &&
             * Biomes.promisedLandPlains.isPresent() && Biomes.promisedLandShrub.isPresent())
             * {
             * EntityRegistry.addSpawn(EntityBird.class, 10, 3, 5, EnumCreatureType.ambient,
             * Biomes.promisedLandForest.get(), Biomes.promisedLandSwamp.get(), Biomes.promisedLandPlains.get(),
             * Biomes.promisedLandShrub.get());
             * }
             */
        }
