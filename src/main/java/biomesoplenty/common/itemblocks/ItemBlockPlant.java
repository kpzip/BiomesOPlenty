...
public class ItemBlockPlant extends ItemBlock {
    ...
    public IIcon getIconFromDamage(int meta) {
        if (meta == 6) return textures[0];
        else if (meta == 7) return textures[1];
        else if (meta == 8) return textures[2];
        else if (meta == 13) return textures[3];
        else if (meta == 14) return textures[4];
        else if (meta == 15) return textures[5];
        else
            // TODO: block getIcon()
    ...
    public ItemStack onItemRightClick(ItemStack itemStack, World world, EntityPlayer player) {
        if (itemStack.getItemDamage() != 14) return super.onItemRightClick(itemStack, world, player);
        ...
        if (movingobjectposition == null) return itemStack;
            ...
            if (movingobjectposition.typeOfHit == MovingObjectType.BLOCK) {
                ...
                if (!world.canMineBlock(player, i, j, k)) return itemStack;

                if (!player.canPlayerEdit(i, j, k, movingobjectposition.sideHit, itemStack)) return itemStack;
