...
public class ItemBlockGrave extends ItemBlock {
    ...
    private boolean placeGrave(ItemStack itemstack, EntityPlayer player, Block block, World world, int x, int y, int z,
        int meta, int side, float hitVecX, float hitVecY, float hitVecZ) {
        // TODO: isAirBlock()
        if (world.isAirBlock(x, y + 1, z)) {
            ...
            if (placeBlockAt(itemstack, player, world, x, y, z, side, hitVecX, hitVecY, hitVecZ, k1)
                && placeBlockAt(itemstack, player, world, x, y + 1, z, side, hitVecX, hitVecY, hitVecZ, k2)) {
                // TODO: linkedBlock stepSound.getPlaceSound() linkedBlock stepSound.getVolume() linkedBlock
                // stepSound.getPitch()
