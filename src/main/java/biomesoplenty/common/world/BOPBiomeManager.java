...
public class BOPBiomeManager {
    ...
    public static BiomeGenBase createAndRegisterBiome(Class<? extends BiomeGenBase> biomeClass, String biomeType,
        String biomeName, List<BiomeEntry> biomeList, int weight) {
        ...
        if (biome != null) {
            ...
            if (BOPConfigurationBiomeGen.config.get(biomeType + " Biomes To Generate", biome.biomeName, true)
                .getBoolean(false)) {
    ...
    public static BiomeGenBase createBiome(Class<? extends BiomeGenBase> biomeClass, String biomeName, int biomeId) {
        if (biomeId == -1)
            biomeId = BOPConfigurationIDs.config.get("Biome IDs", biomeName + " ID", getNextFreeBiomeId())
                .getInt();
        ...
        if (biomeId != -1) {
            try {
                ...
                if (BOPInheritedBiome.class.isAssignableFrom(biomeClass)) {
                    biome = biomeClass.getConstructor(int.class, BiomeGenBase.class)
                        .newInstance(biomeId, BiomeGenBase.getBiome(biomeId))
                        .setBiomeName(biomeName);
                } else {
                    biome = biomeClass.getConstructor(int.class)
                        .newInstance(biomeId)
                        .setBiomeName(biomeName);
                }
    ...
    private static int getConfiguredWeight(BiomeGenBase biome, String biomeType, int weight) {
        return BOPConfigurationBiomeWeights.config.get(biomeType + " Biome Weights", biome.biomeName, weight)
            .getInt(weight);
    }
