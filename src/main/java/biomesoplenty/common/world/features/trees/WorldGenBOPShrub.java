...
public class WorldGenBOPShrub extends WorldGenTrees {
    ...
    public WorldGenBOPShrub(Block wood, Block leaves, int metaWood, int metaLeaves, int minHeight, int maxHeight,
        Block... soilBlocks) {
        super(false);
                ...
                for (int l = y; l <= y + 2; ++l) {
                    ...
                    for (int k1 = x - j1; k1 <= x + j1; ++k1) {
                        ...
                        for (int i2 = z - j1; i2 <= z + j1; ++i2) {
                            ...
                            if ((Math.abs(l1) != j1 || Math.abs(j2) != j1 || random.nextInt(2) != 0)
