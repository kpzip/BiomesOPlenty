...
public class WorldGenBOPTallGrass extends WorldGeneratorBOP {
    ...
    public boolean generate(World world, Random random, int x, int y, int z) {
        ...
        for (int l = 0; l < 128; ++l) {
            ...
            if (world.isAirBlock(i1, j1, k1) && (this.tallGrass
                .canReplace(world, i1, j1, k1, 0, new ItemStack(this.tallGrass, 1, this.tallGrassMetadata)))) {
    ...
    public void setupGeneration(World world, Random random, BOPBiome biome, String featureName, int x, int z) {
        for (int i = 0; i < (Integer) biome.theBiomeDecorator.bopFeatures.getFeature(featureName); i++) {
            ...
            int randY = world.provider.isHellWorld ? random.nextInt(128)
                : random.nextInt(world.getHeightValue(randX, randZ) * 2);
