...
public class WorldGenKelp extends WorldGeneratorBOP {
    ...
    public boolean generate(World world, Random rand, int x, int y, int z) {
        ...
        if (world.getBlock(x2, y2, z2) == Blocks.water && world.getBlock(x2, y2 + 1, z2) == Blocks.water
            && coral.canBlockStay(world, x2, y, z2, 8)) {
            for (int i = 0; i < height; ++i) {
                currentMeta = i == 0 ? 8 : i == height - 1 ? 10 : 9;

                if (world.getBlock(x2, y2 + i, z2) == Blocks.water
                    && world.getBlock(x2, y2 + i + 1, z2) == Blocks.water) {
