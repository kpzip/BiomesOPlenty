...
public class WorldGenWaspHive extends WorldGeneratorBOP {
    ...
    public boolean generate(World world, Random rand, int x, int y, int z) {
        ...
        for (int cubeno = 0; cubeno < 4; cubeno++) {
            ...
            // Top
            generateHiveCubeSmall(
                world,
                x,
                y + cubeno,
                z,
                (baseHeight - 11) + (cubeno * 2),
                (baseWidth - 1) + cubeno,
                cubeno,
                chance,
                meta);
            ...
            generateHiveCubeSmall(
                world,
                x,
                (y - (baseHeight + 6)) + cubeno,
                z,
                (baseHeight - 10) + (cubeno * 2),
                (baseWidth - 1) + cubeno,
                cubeno,
                chance,
                meta);
            ...
            generateHiveCubeSmall(
                world,
                x,
                (y - (baseHeight + 7)) + cubeno,
                z,
                (baseHeight - 9) + (cubeno * 2),
                (baseWidth - 2) + cubeno,
                cubeno,
                chance,
                meta);
    ...
    public void generateHiveCube(World world, int origx, int origy, int origz, int height, int width, int cubeno,
        float chance, int honeychance, int meta) {
        for (int hLayer = 0; hLayer < height; hLayer++) {
            for (int i = -width; i < width; i++) {
                for (int j = -width; j < width; j++) {
                    if ((hLayer == 0 || hLayer == (height - 1)) && (world.rand.nextFloat() <= chance))
                        world.setBlock(origx + i, origy - hLayer, origz + j, BOPCBlocks.hive, meta, 2);
                    ...
                    if (hLayer > (height / 2)) {
                        if (honeychance == 0) {
                            if (cubeno < 2 && world.getBlock(origx + i, origy - hLayer, origz + j) != BOPCBlocks.hive)
                                world.setBlock(origx + i, origy - hLayer, origz + j, BOPCBlocks.honey, 7, 2);
                            if (cubeno < 2 && world.getBlock(origx + i, origy - hLayer, origz + j) == BOPCBlocks.hive
                                && world.getBlockMetadata(origx + i, origy - hLayer, origz + j) != 0)
                                world.setBlock(origx + i, origy - hLayer, origz + j, BOPCBlocks.honey, 7, 2);
                        } else {
                            if (cubeno < 2 && world.getBlock(origx + i, origy - hLayer, origz + j) != BOPCBlocks.hive)
                                world.setBlockToAir(origx + i, origy - hLayer, origz + j);
                            if (cubeno < 2 && world.getBlock(origx + i, origy - hLayer, origz + j) == BOPCBlocks.hive
                                && world.getBlockMetadata(origx + i, origy - hLayer, origz + j) != 0)
                                world.setBlockToAir(origx + i, origy - hLayer, origz + j);
                        }
                        ...
                        if (honeychance == 0) {
                            if (hLayer == (height / 2)) {
                                if (cubeno < 2
                                    && world.getBlock(origx + i, origy - hLayer, origz + j) != BOPCBlocks.hive)
                                    world.setBlock(origx + i, origy - hLayer, origz + j, BOPCBlocks.honeyBlock, 0, 2);
                                if (cubeno < 2
                                    && world.getBlock(origx + i, origy - hLayer, origz + j) == BOPCBlocks.hive
                                    && world.getBlockMetadata(origx + i, origy - hLayer, origz + j) != 0)
                                    world.setBlock(origx + i, origy - hLayer, origz + j, BOPCBlocks.honeyBlock, 0, 2);
    ...
    public void generateHiveCubeSmall(World world, int origx, int origy, int origz, int height, int width, int cubeno,
        float chance, int meta) {
        for (int hLayer = 0; hLayer < height; hLayer++) {
            for (int i = -width; i < width; i++) {
                for (int j = -width; j < width; j++) {
                    if ((hLayer == 0 || hLayer == (height - 1)) && (world.rand.nextFloat() <= chance))
                        world.setBlock(origx + i, origy - hLayer, origz + j, BOPCBlocks.hive, 1, 2);
                    else if ((i == -width || i == (width - 1) || j == -width || j == (width - 1))
                        && (world.rand.nextFloat() <= chance))
                        world.setBlock(origx + i, origy - hLayer, origz + j, BOPCBlocks.hive, 1, 2);
                }
        ...
        for (int spawn = 0; spawn < 50; spawn++) {
            ...
            if (world.getBlock(spawnx, spawny, spawnz) == BOPCBlocks.hive) {
                if (world.getBlockMetadata(spawnx, spawny, spawnz) == 1) {
                    if (world.getBlock(spawnx - 1, spawny, spawnz) == BOPCBlocks.hive
                        && world.getBlock(spawnx + 1, spawny, spawnz) == BOPCBlocks.hive
                        && world.getBlock(spawnx, spawny, spawnz - 1) == BOPCBlocks.hive
                        && world.getBlock(spawnx, spawny, spawnz + 1) == BOPCBlocks.hive
                        && world.getBlock(spawnx, spawny - 1, spawnz) == BOPCBlocks.hive
                        && world.getBlock(spawnx, spawny + 1, spawnz) == BOPCBlocks.hive) {
                        if (world.getBlockMetadata(spawnx - 1, spawny, spawnz) == 1
                            && world.getBlockMetadata(spawnx + 1, spawny, spawnz) == 1
                            && world.getBlockMetadata(spawnx, spawny, spawnz - 1) == 1
                            && world.getBlockMetadata(spawnx, spawny, spawnz + 1) == 1
                            && world.getBlockMetadata(spawnx, spawny - 1, spawnz) == 1
                            && world.getBlockMetadata(spawnx, spawny + 1, spawnz) == 1) {
                            ...
                            TileEntityMobSpawner tileentitymobspawner = (TileEntityMobSpawner) world
                                .getTileEntity(spawnx, spawny, spawnz);

                            if (tileentitymobspawner != null) {
                                // getSpawnerLogic()
                                tileentitymobspawner.func_145881_a()
                                    .setEntityName("BiomesOPlenty.Wasp");
                            }
                        }
                    }
                }
