...
public class GenLayerSubBiome extends GenLayer {
        ...
        for (int xi = 0; xi < width; ++xi) {
            for (int zi = 0; zi < length; ++zi) {
                ...
                BOPSubBiome selectedSubBiome = currentSubBiomes != null
                    ? (BOPSubBiome) currentSubBiomes.get(this.nextInt(currentSubBiomes.size())).biome
                    : null;

                if (selectedSubBiome != null) {
                    Pair<Integer, Integer> offset = getOffset(selectedSubBiome);

                    if (SimplexNoise.noise(
                        (xi + x + offset.getLeft()) * selectedSubBiome.zoom,
                        (zi + z + offset.getRight()) * selectedSubBiome.zoom) > selectedSubBiome.threshold) {
    ...
    private void setOffsets() {
        for (int i = 0; i < offsets.length; i++) {
            offsets[i] = Pair
                .of(this.nextInt(OFFSET_RANGE) - (OFFSET_RANGE / 2), this.nextInt(OFFSET_RANGE) - (OFFSET_RANGE / 2));
        }
    }
}
