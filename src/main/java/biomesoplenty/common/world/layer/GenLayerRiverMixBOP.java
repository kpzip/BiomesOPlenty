...
public class GenLayerRiverMixBOP extends GenLayerRiverMix {
    ...
    public int[] getInts(int x, int z, int width, int length) {
        ...
        for (int i1 = 0; i1 < width * length; ++i1) {
            if (inputBiomeIds[i1] != BiomeGenBase.ocean.biomeID
                && inputBiomeIds[i1] != BiomeGenBase.deepOcean.biomeID) {
                if (riverBiomeIds[i1] == BiomeGenBase.river.biomeID) {
                    if (inputBiomeIds[i1] == BiomeGenBase.icePlains.biomeID
                        || inputBiomeIds[i1] == BOPCBiomes.alps.biomeID
                        || inputBiomeIds[i1] == BOPCBiomes.alpsForest.biomeID
                        || inputBiomeIds[i1] == BOPCBiomes.arctic.biomeID
                        || inputBiomeIds[i1] == BOPCBiomes.glacier.biomeID
                        || inputBiomeIds[i1] == BOPCBiomes.frostForest.biomeID
                        || inputBiomeIds[i1] == BOPCBiomes.snowyConiferousForest.biomeID) {
                    ...
                    } else if (inputBiomeIds[i1] != BiomeGenBase.mushroomIsland.biomeID
                        && inputBiomeIds[i1] != BiomeGenBase.mushroomIslandShore.biomeID) {
