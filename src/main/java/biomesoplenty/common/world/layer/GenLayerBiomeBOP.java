...
public class GenLayerBiomeBOP extends GenLayerBiome {
    ...
    public List<BiomeEntry>[] biomeLists = new ArrayList[] { new ArrayList(), new ArrayList(), new ArrayList(),
        new ArrayList() };
        ...
        for (int i1 = 0; i1 < length; ++i1) {
            for (int j1 = 0; j1 < width; ++j1) {
                ...
                if (isBiomeOceanicAndEnabled(currentBiomeID)) {
                    ...
                    && BOPConfigurationBiomeGen.mushroomIslandGen) {
                        outputBiomeIDs[j1 + i1 * width] = currentBiomeID;
    ...
    private int getBiomeIdFromMixedList(int... listIdExclusions) {
        ...
        for (int i = 0; i < 4; i++) {
            if (!listIdExclusionList.contains(i) && !this.biomeLists[i].isEmpty())
    ...
    private int getWeightedBiomeFromList(List<BiomeEntry> biomeList) {
        return ((BiomeEntry) WeightedRandom
            .getItem(biomeList, (int) this.nextLong(WeightedRandom.getTotalWeight(biomeList) / 10) * 10)).biome.biomeID;
    }

    private boolean isBiomeOceanicAndEnabled(int biomeId) {
        return (biomeId == BiomeGenBase.ocean.biomeID && BOPConfigurationBiomeGen.oceanGen)
            || (biomeId == BiomeGenBase.deepOcean.biomeID && BOPConfigurationBiomeGen.deepOceanGen)
            || (biomeId == BiomeGenBase.frozenOcean.biomeID && BOPConfigurationBiomeGen.frozenOceanGen);
    }
}
